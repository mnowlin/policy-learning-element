...
flowchart LR
    ...
    subgraph1 --> D[Policy Preferences]
```
...
%%| label: fig-updating
%%{init: {'theme':'neutral' , "htmlLabels": true, "securityLevel": "loose",}}%%
block-beta
   columns 3
   A space  B 
   space space space
   ...
   A["Policy Core <br> <i>Issue Defintion<sub>1</sub>"] --> B["Policy <br> Preference<sub>1</sub>"]
   A -- "<i>Information <br> Processing" ------> C
   C["Policy Core <br> <i>Issue Defintion<sub>2</sub>"] --> D["Policy <br> Preference<sub>2</sub>"]
```
...
The data comes from a series of surveys ... 
